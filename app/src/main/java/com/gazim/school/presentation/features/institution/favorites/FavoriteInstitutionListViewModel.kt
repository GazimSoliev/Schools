...
import com.gazim.school.domain.usecases.IGetFavoriteInstitutionsUseCase
import com.gazim.school.domain.usecases.IGetFilteredFavoriteInstitutionsUseCase
...
class FavoriteInstitutionListViewModel(
    ...
    private fun getData(request: String) = intent {
        ...
        when (request.isBlank()) {
            true -> {
                getFavoriteInstitutionsUseCase.getFavoriteInstitutions().onSuccess { result ->
                    reduce {
                        state.copy(
